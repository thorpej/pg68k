...
initial begin
	$dumpfile("mmu_rmw_super_prot.vcd");
	$dumpvars;
...
initial begin
	$display("MMU TEST: Supervisor R-M-W cycle, prot error.");
	...
	if (~n_as_out) begin
		$fatal(1, "FATAL: MMU failed to gate /AS.");
	end
	if (n_berr_out) begin
	...
	if (~n_uds_out) begin
		$fatal(1, "FATAL: MMU failed to gate /UDS.");
	end
